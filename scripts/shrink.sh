...
USER="macramole"                     # linux user name
IMAGE_NAME="aaeeRaspi"          # image name, alternative with date and time: "image_$(date +"%y%m%d%H%M%S")"
IMAGE="${IMAGE_NAME}.img"   # image name with extension
LOOP=$(losetup -f)

READ=false                   # read image from SD card (false for an already existing image)
RESIZE=true                 # resize image with GParted
FILL=false                   # fill empty space of new image with zeroes, only possible if RESIZE=true
